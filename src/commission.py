...
class CommissionCalculator:
    ...
    def _get_effective_fees(self, broker_key: str, asset_type: AssetType) -> Dict[str, Any]:
        ...
        try:
            ...
            if "not found" in str(e):
                 raise ValueError(f"Broker key '{broker_key}' not found in master schedule.") from e
            raise e
        ...
        effective_config = UtilsJson.deep_merge(effective_config, merged_asset_config)

        # 3. Final validation
        if not effective_config.get("broker") and not effective_config.get("regulatory"):
             raise ValueError(f"Asset type '{asset_key}' configuration not found in schedule for broker '{broker_key}'.")
    ...
    def _calculate_stocks_fees(self, principal_value: float, trade_side: TradeSide, holding_type: HoldingType, rates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        percentage_brokerage = principal_value * broker_rates.get(rate_key, 0.0)
        ...
        stamp_duty = principal_value * regulatory_rates.get("stamp_duty_rate", 0.0) if trade_side == TradeSide.BUY else 0.00
        ...
        if holding_type == HoldingType.DELIVERY and trade_side == TradeSide.SELL:
            stt_tax = principal_value * regulatory_rates.get("stt_delivery", 0.0)
        elif holding_type == HoldingType.INTRADAY:
            # For intraday, STT is applied on both sides in this model's logic
            stt_tax = principal_value * regulatory_rates.get("stt_intraday", 0.0)
        ...
        fees_breakdown = {
            ...
            "Holding Type": holding_type.name, # Using .name (DELIVERY, INTRADAY) for display
    ...
    def calculate_commission_fees(
        ...
        holding_type: HoldingType = self._determine_holding_type(buy_datetime, sell_datetime)
